...
//    event->pid = bpf_get_current_pid_tgid() >> 32;
    ...
    member_read(&_skb_refdst, skb, _skb_refdst);
	struct dst_entry *dst = (struct dst_entry *)(_skb_refdst & SKB_DST_PTRMASK);
	struct rtable *rt = (struct rtable *)dst;
    member_read(&event->nexthop, rt, rt_gateway);
    ...
    bpf_probe_read(&event->src_mac, 6, l2_header_address + 6); // src offset 6B

//    struct ethhdr ethhdr;
//    bpf_probe_read(&ethhdr, sizeof(ethhdr), l2_header_address);
//    event->dest_mac = ethhdr.h_dest;
//    event->src_mac = ethhdr.h_source;
...
static int
do_trace_sk(void *ctx, struct sk_buff *skb, const char *func_name, void *netdev)
{
    struct event_t event = {};
    union ___skb_pkt_type type = {};
...
// arp
int kprobe__arp_xmit(struct pt_regs *ctx, struct sk_buff *skb)
{
   return do_trace(ctx, skb, __func__+8, NULL);
}
 ...
 * 2) dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev, struct netdev_queue *txq)
 * 3) loopback_xmit(struct sk_buff *skb, struct net_device *sb_dev)
 * 4) ...
 */
...
// 抓接口发出包的点
int kprobe__dev_hard_start_xmit(struct pt_regs *ctx, struct sk_buff *skb, struct net_device *sb_dev, struct netdev_queue *txq, int *ret)
{
   return do_trace(ctx, skb, __func__+8, NULL);
}

int kprobe__loopback_xmit(struct pt_regs *ctx, struct sk_buff *skb, struct net_device *sb_dev)
{
   return do_trace(ctx, skb, __func__+8, NULL);
}
...
#if 1
// 1. Attach kprobe to "do_syscall_64",need filter TODO filter
int kprobe__do_syscall_64(struct pt_regs *ctx, int nr)
{
    struct event_t event = {};
    union ___skb_pkt_type type = {};
    struct sk_buff *skb=(void*)ctx->bx;
...
//  event_printer only support 4、6
    event.ip_version = 4;
