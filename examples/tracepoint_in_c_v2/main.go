...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -type event bpf tracepoint.c -- -I../headers
...
func main() {
	...
	// /sys/kernel/tracing/events/sched/sched_process_exec
	...
	for {
		...
		log.Printf("pid: %d\tcomm: %s\tfilename: %s\n", event.Pid, unix.ByteSliceToString(event.Comm[:]), event.Filename)
	}
}
